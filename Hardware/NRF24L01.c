...
/**
  ...
  * 参    数：无
  * 返 回 值：无
  * 说    明：配置 CE/CSN 为推挽输出，SCK/MOSI/MISO 为 SPI1 复用功能
  *           SPI1 重映射到 PB3(SCK) PB4(MISO) PB5(MOSI)
  */
void NRF24L01_GPIO_Init(void)
{
	/*开启AFIO时钟，禁用JTAG释放PB3/PB4/PA15，重映射SPI1到PB3/PB4/PB5*/
	...
	/*CE: PA8, CSN: PA15 —— 推挽输出*/
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_PP;
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_8 | GPIO_Pin_15;
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
	GPIO_Init(GPIOA, &GPIO_InitStructure);

	/*SCK: PB3, MOSI: PB5 —— 复用推挽输出（硬件SPI控制）*/
...
/**
  * 函    数：NRF24L01硬件SPI初始化
  * 参    数：无
  * 返 回 值：无
  * 说    明：配置 SPI1 —— 主机模式、全双工、8位、MSB先行、SPI模式0(CPOL=0,CPHA=0)
  *           时钟 9MHz (72MHz/8)，软件NSS（由CSN引脚手动控制）
  */
