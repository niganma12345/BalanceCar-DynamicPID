...
/**
  ...
  * 说    明：使用差值法计算增量，不清零 CNT，避免丢失两次调用之间的脉冲
  *           16 位自然溢出回绕由补码减法自动处理
  */
int16_t Encoder_Get(uint8_t n)
{
	static uint16_t LastCount[2] = {0, 0};	//静态变量，记录左、右编码器上次的 CNT 值
	uint16_t Now;
	int16_t Delta;
	...
	if (n == 1)					//指定左电机
	{
		...
		Delta = (int16_t)(Now - LastCount[0]);	//补码减法自动处理 16 位溢出回绕
		LastCount[0] = Now;
		return Delta;
	}
	else if (n == 2)			//指定右电机
